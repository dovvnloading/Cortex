...
This document is background research and a target design, not policy: per
[`AGENTS.md`](../AGENTS.md) ("Anything under `docs/` is background and design
notes, not policy: this file is the only normative contract"), `AGENTS.md`
governs whenever the two disagree. This document records the cross-harness
capability standard and the evidence that would be required before expanding
Cortex's authority; the detailed delivery backlog it once split out to
`CODING_AGENT_HARDENING_ROADMAP.md` is folded into the staged delivery plan
below and into [`docs/NEXT.md`](NEXT.md).
