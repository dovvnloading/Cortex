...
from ui_widgets import (
    ...
    ConfirmDeleteDialog, RenameDialog
)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, orchestrator, chat_models: list[str]):
        ...
        # Initialize the blur effect for modal dialogs.
        ...
        self.blur_ref_count = 0  # To handle nested dialogs correctly
    ...
    def set_blur(self, enabled: bool):
        ...
        Uses a reference counter to handle cases where multiple dialogs
        that trigger the blur might be open (e.g., settings -> memory management).

        Args:
            enabled (bool): True to apply the blur, False to remove it.
        """
        if enabled:
            if self.blur_ref_count == 0:
                # Animate from current radius to the target blur radius.
                self.blur_animation.setStartValue(self.blur_effect.blurRadius())
                self.blur_animation.setEndValue(8)
                self.blur_animation.start()
            self.blur_ref_count += 1
        else:
            self.blur_ref_count -= 1
            if self.blur_ref_count == 0:
                # Animate from current radius back to zero.
                self.blur_animation.setStartValue(self.blur_effect.blurRadius())
                self.blur_animation.setEndValue(0)
                self.blur_animation.start()
            
            # Failsafe to prevent negative reference count.
            if self.blur_ref_count < 0:
                self.blur_ref_count = 0
    ...
    def apply_theme(self, theme_name: str):
        ...
        # Re-polish all child widgets to ensure they pick up the new theme styles.
    ...
    def on_send(self):
        ...
        self.loading_widgets[active_thread_id] = loading_widget
        
        self.set_chat_ui_for_processing(True)

        # Offload the AI query to a separate worker thread to avoid freezing the UI.
        ...
        self.query_worker.thread_id = active_thread_id
    ...
    def append_message(self, role, sender, message, sources=None, thoughts=None) -> ChatMessageWidget:
        ...
        Args:
            ...
            message (str): The message content (can be plain text or Markdown).
        ...
        """
        # The 'message' parameter is the plain text.
        plain_text_content = message
        
        # 'display_content' will hold the formatted version for the QLabel.
        display_content = message
        if role == 'assistant':
            # Render Markdown for assistant responses.
            display_content = markdown.markdown(message, extensions=['fenced_code', 'tables', 'nl2br'])
        elif role == 'user':
            # Convert newlines to <br> for HTML display in QLabel.
            display_content = message.replace('\n', '<br>')
        
        # Pass the plain text to the widget's constructor, but display the formatted version.
        message_widget = ChatMessageWidget(role, sender, display_content, sources=sources, thoughts=thoughts)
        message_widget.plain_text = plain_text_content # Explicitly set the plain_text attribute
