...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.context_menu = CustomContextMenu(self.central_widget)
    ...
    def eventFilter(self, watched: QObject, event: QEvent) -> bool:
        ...
        """
        if event.type() == QEvent.MouseButtonPress and self.context_menu.isVisible():
            if not self.context_menu.geometry().contains(self.central_widget.mapFromGlobal(event.globalPos())):
    ...
    def append_message(self, role, sender, message, sources=None, thoughts=None) -> ChatMessageWidget:
        ...
        if role == 'assistant':
            ...
            content = markdown.markdown(message, extensions=['fenced_code', 'tables', 'nl2br'])
        elif role == 'user':
            # Convert newlines to <br> for HTML display in QLabel.
            content = message.replace('\n', '<br>')
        
        message_widget = ChatMessageWidget(role, sender, content, sources=sources, thoughts=thoughts)
    ...
    def on_show_context_menu(self, thread_id: str, global_pos: QPoint):
        ...
        rename_action = self.context_menu.addAction("Rename")
        ...
        self.context_menu.show_at(local_pos)
