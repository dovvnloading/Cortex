...
from PySide6.QtGui import QPainter, QColor, QPainterPath, QClipboard
...
class ChatMessageWidget(QWidget):
    ...
    loading indicators.
    """
    def __init__(self, msg_type, sender, text, sources=None, thoughts=None):
        """
        ...
        if msg_type == 'system':
            ...
            self._create_chat_bubble(bubble_row_layout, sender, text)
            main_layout.addLayout(bubble_row_layout)
    ...
    def show_context_menu(self, pos: QPoint):
        ...
        # Parent the menu to the main window to ensure it's not clipped and is styled correctly.
        ...
        copy_action.setEnabled(self.message_label.hasSelectedText())
        copy_action.clicked.connect(lambda: QApplication.clipboard().setText(self.message_label.selectedText()))
        ...
        global_pos = self.message_label.mapToGlobal(pos)
        # Map the global position to the main window's coordinate system for correct placement.
    ...
    def _create_system_message(self, main_layout, text):
        ...
        system_frame = QFrame()
        system_frame.setMinimumWidth(400)
    ...
    def _create_chat_bubble(self, bubble_row_layout, sender, text):
        """Constructs the UI for 'user' and 'assistant' message bubbles."""
        # Use the stored plain_text for logic, but the passed 'text' (which becomes formatted_text) for display.
        formatted_text = text
        bubble_container = QWidget()
        bubble_layout = QVBoxLayout(bubble_container)
        bubble_layout.setContentsMargins(0, 0, 0, 0)
        bubble_layout.setSpacing(4)
        bubble_container.setMinimumWidth(400)
        ...
        # Apply different styles for user vs. assistant bubbles.
        if is_user:
            bubble.setStyleSheet("""
                QFrame#MessageBubble {
        ...
        bubble_inner_layout.setSpacing(0)
        
        self.message_label = QLabel(formatted_text)
        self.message_label.setObjectName("MessageContent")
        self.message_label.setWordWrap(True)
        ...
        user_message_style = "color: #ffffff; font-size: 14px; background: transparent; border: none; line-height: 1.6;"
        ai_message_style = """
            ...
            QLabel#MessageContent pre { background-color: #2d2d2d; color: #f5f1ed; border-radius: 8px; padding: 14px; font-family: 'SF Mono', 'Consolas', monospace; font-size: 13px; line-height: 1.6; overflow-x: auto; white-space: pre-wrap; }
            ...
            *[theme="dark"] QLabel#MessageContent pre { background-color: #1e1e1e; color: #d4d4d4; }
        """
        self.message_label.setStyleSheet(user_message_style if is_user else ai_message_style)
        bubble_inner_layout.addWidget(self.message_label)
        
        # Add sources/thoughts sections only for AI messages that have them.
        if not is_user and (self.sources or self.thoughts):
            separator = QFrame()
...
class BaseDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.main_layout.addWidget(dialog_frame)

        self.frame_layout = QVBoxLayout(dialog_frame)
...
class BlurringBaseDialog(BaseDialog):
    """
    A base dialog that applies a blur effect to its parent QMainWindow upon showing.
    
    This class finds the root QMainWindow and calls its `set_blur` method.
    """
    def __init__(self, parent=None):
        super().__init__(parent)
    ...
    def exec(self):
        """
        Overrides `exec` to apply blur before showing and remove it after closing.
        """
        if self._main_window and hasattr(self._main_window, 'set_blur'):
            self._main_window.set_blur(True)
        
        try:
            # Call the original exec method to show the dialog modally.
            result = super().exec()
        finally:
            # This 'finally' block ensures the blur is always removed, even if an error occurs.
            if self._main_window and hasattr(self._main_window, 'set_blur'):
                self._main_window.set_blur(False)
